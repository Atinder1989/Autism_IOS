//
//  LearningFineMotorMovementViewModel.swift
//  Autism
//
...
class LearningFineMotorMovementViewModel: NSObject {
    ...
    var childActionStateClosure : ((Bool,ScriptCommandInfo) -> Void)?
    ...
    private var currentCommandIndex = 2 {
        didSet{
            self.executeCommand()
        }
    }
...
extension LearningFineMotorMovementViewModel {
    ...
    private func handleChildActionState(state:Bool,commandInfo:ScriptCommandInfo?) {
        if state {
            if let info = commandInfo {
                ...
                SpeechManager.shared.speak(message: info.value, uttrenceRate: AppConstant.speakUtteranceNormalRate.rawValue.floatValue)
            }
...
extension LearningFineMotorMovementViewModel: SpeechManagerDelegate {
...
extension LearningFineMotorMovementViewModel: ScriptManagerDelegate {
...
extension LearningFineMotorMovementViewModel: PlayerControllerDelegate {
