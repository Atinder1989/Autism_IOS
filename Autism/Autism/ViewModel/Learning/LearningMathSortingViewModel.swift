//
//  LearningMathSortingViewModel.swift
//  Autism
//
...
class LearningMathSortingViewModel: NSObject {
    ...
    var showFoilImageClosure : ((_ questionInfo:ScriptCommandInfo) -> Void)?
...
extension LearningMathSortingViewModel {
    ...
    private func handleShowFoilImageCommand(commandInfo:ScriptCommandInfo) {
        if let closure = self.showFoilImageClosure {
            closure(commandInfo)
        }
    }
...
extension LearningMathSortingViewModel: SpeechManagerDelegate {
...
extension LearningMathSortingViewModel: ScriptManagerDelegate {
    func get(scriptCommand: ScriptCommand) {
        ...
        case .show_image(commandInfo:  let commandInfo):
            ...
            break
        case .show_foil_image(commandInfo: let commandInfo):
            if let info = commandInfo {
                self.handleShowFoilImageCommand(commandInfo: info)
            }
...
extension LearningMathSortingViewModel: PlayerControllerDelegate {
