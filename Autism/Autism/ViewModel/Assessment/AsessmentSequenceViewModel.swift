//
//  AsessmentSequenceViewModel.swift
//  Autism
//
...
class AsessmentSequenceViewModel:NSObject  {
    
    var submitAnswerClosure : (() -> Void)?
    
    var accessmentSubmitResponseVO: AssessmentQuestionResponseVO? = nil {
        didSet {
            if let closure = self.submitAnswerClosure {
                closure()
            }
        }
    }
    ...
    func submitUserAnswer(completeRate:Int,info:SequenceResponseInfo,timeTaken:Int,skip:Bool) {
