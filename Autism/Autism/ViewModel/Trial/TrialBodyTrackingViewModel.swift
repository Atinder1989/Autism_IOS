//
//  TrialBodyTrackingViewModel.swift
//  Autism
//
...
import Foundation
class TrialBodyTrackingViewModel: NSObject {
    ...
    var decreaseSpeedClosure : ((_ questionInfo:ScriptCommandInfo) -> Void)?
    var showFingerOnImage : ((_ questionInfo:ScriptCommandInfo) -> Void)?
    var haltScreen : ((_ questionInfo:ScriptCommandInfo) -> Void)?
    var popBalloon : ((_ questionInfo:ScriptCommandInfo) -> Void)?
    var avatar : ((_ questionInfo:ScriptCommandInfo) -> Void)?
    var zommOnAvatar : ((_ questionInfo:ScriptCommandInfo) -> Void)?
    //zomm_on_avatar
    ...
    func submitFaceTrackDetails(info:BodyTrackingQuestionInfo,completeRate:Int,timetaken:Int,skip:Bool,touchOnEmptyScreenCount:Int) {
...
extension TrialBodyTrackingViewModel {
    private func executeCommand() {
        if let commandResponseVO = self.BodyTrackingQuestionInfo  {
...
//        }
    ...
    private func handleDecreaseSpeedCommand(commandInfo:ScriptCommandInfo) {
        if let closure = self.decreaseSpeedClosure {
            closure(commandInfo)
        }
    }
    
    private func handlShowFingerOnImageCommand(commandInfo:ScriptCommandInfo) {
        if let closure = self.showFingerOnImage {
            closure(commandInfo)
        }
    }
    
    private func handlHaltScreenCommand(commandInfo:ScriptCommandInfo) {
        if let closure = self.haltScreen {
            closure(commandInfo)
        }
    }
    private func handlPopBalloonCommand(commandInfo:ScriptCommandInfo) {
        if let closure = self.popBalloon {
            closure(commandInfo)
        }
    }
    
    private func handlAvatarCommand(commandInfo:ScriptCommandInfo) {
        if let closure = self.avatar {
            closure(commandInfo)
        }
    }
    private func handlZommOnAvatarCommand(commandInfo:ScriptCommandInfo) {
        if let closure = self.zommOnAvatar {
            closure(commandInfo)
        }
    }
...
extension TrialBodyTrackingViewModel: SpeechManagerDelegate {
...
extension TrialBodyTrackingViewModel: ScriptManagerDelegate {
    func get(scriptCommand: ScriptCommand) {
        ...
        case .decrease_speed(commandInfo: let commandInfo):
            if let info = commandInfo {
                self.handleDecreaseSpeedCommand(commandInfo: info)
            }
        case .show_finger_on_image(commandInfo: let commandInfo):
            if let info = commandInfo {
                self.handlShowFingerOnImageCommand(commandInfo: info)
            }
        case .halt_screen(commandInfo: let commandInfo):
            if let info = commandInfo {
                self.handlHaltScreenCommand(commandInfo: info)
            }
        case .avatar(commandInfo: let commandInfo):
            if let info = commandInfo {
                self.handlAvatarCommand(commandInfo: info)
            }
        case .zomm_on_avatar(commandInfo: let commandInfo):
            if let info = commandInfo {
                self.handlZommOnAvatarCommand(commandInfo: info)
            }
        case .pop_balloon(commandInfo: let commandInfo):
            if let info = commandInfo {
                self.handlPopBalloonCommand(commandInfo: info)
            }
